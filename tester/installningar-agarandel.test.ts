...
// granssnittet"). Samma mockningsmonster som upplatelseform-byte.test.ts.
...
describe("agarandel valideras aven pa servern", () => {
  it("avvisar 1000 – det klassiska skrivfelet som skulle gora avdraget tio ganger for stort", async () => {
    const resultat = await sparaInstallningar({}, formulardata({ agarandel: "1000" }));
  ...
  it("avvisar noll", async () => {
    const resultat = await sparaInstallningar({}, formulardata({ agarandel: "0" }));
  ...
  it("sparar ett giltigt decimalvarde", async () => {
    const resultat = await sparaInstallningar({}, formulardata({ agarandel: "33,33" }));
  ...
  it("tolkar ett tomt falt som hela bostaden, 100", async () => {
    const resultat = await sparaInstallningar({}, formulardata({ agarandel: "" }));
